...
import type { ModuleType, ModuleData } from '../../store/workspaceStore';
import clsx from 'clsx';
import { BoxSelect } from 'lucide-react';

function ContainerNode({ data, selected }: NodeProps<ModuleData>) {
  const { type, attributeName } = data;
  const color = TYPE_COLORS[type as ModuleType] || '#64748b';
    ...
    >
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-bold tracking-wider" style={{ color }}>{type}</span>
        </div>
      ...
      {/* Target/Source handles for connecting into/out of the container entirely */}
      <Handle 
        type="target" 
